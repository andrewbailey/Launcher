...
data class GridPosition(
    ...
    operator fun plus(other: GridPosition) =
        GridPosition(this.y + other.y, this.x + other.x)

    operator fun plus(other: GridSize) =
        GridPosition(this.y + other.width, this.x + other.height)
}

data class GridSize(
    val width: GridDimension,
    val height: GridDimension
) {
    init {
        require(width >= 0.gd && height >= 0.gd) { "Width and height must be non-negative." }
    }

    operator fun contains(point: GridPosition) =
        point.x >= 0.gd && point.x <= width
            && point.y >= 0.gd && point.y <= height
}

@JvmInline
value class GridDimension(
    val halfSteps: Int
) : Comparable<GridDimension> {
